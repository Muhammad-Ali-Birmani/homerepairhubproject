...
if (isset($_POST['firstname']) && isset($_POST['accountnumber']) && isset($_POST['product_id']) && isset($_POST['price'])) {
  ...
  $firstname = $_POST['firstname'];
    ...
    $customerid = "SELECT C_ID from customer where accountnumber = '".$accountnumber."'";
    $re = sqlsrv_query($conn,$customerid);
    if (!$re) {
          echo "Connection failed"  ;
    }
        ...
        while ($databack = sqlsrv_fetch_array($re, SQLSRV_FETCH_ASSOC)) {
        if (true) {

            $customeridis = $databack['C_ID'];

            $currentDateTime = date("Y-m-d H:i:s");
                
            $InsertInToOrderLine = "INSERT INTO orderline (C_ID ,product_id, orderdate, price)
                            VALUES('$customeridis' , '$productid','$currentDateTime','$productprice')";
            $insertData = sqlsrv_query($conn, $InsertInToOrderLine);
            if (!$insertData) {
                echo "";
}
        }
